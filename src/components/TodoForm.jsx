...
export function TodoForm({ onSubmit, loading }) {
  ...
  const handleSubmit = async (e) => {
    ...
    await onSubmit(title.trim(), description.trim() || undefined, priority)
    setTitle('')
    setDescription('')
    setPriority('medium')
    setIsExpanded(false)
  ...
  return (
    <form onSubmit={handleSubmit} className="backdrop-blur-md bg-white/10 rounded-2xl p-6 border border-white/20 shadow-xl">
          ...
          <div className="space-y-4 animate-in slide-in-from-top-2 duration-200">
            <div>
              <textarea
                ...
                rows={3}
                className="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-lg text-white placeholder-white/60 focus:outline-none focus:ring-2 focus:ring-purple-400/50 focus:border-transparent transition-all duration-200 resize-none"
              />
            </div>
              ...
              <div className="flex space-x-2">
                {['low', 'medium', 'high'].map((p) => {
                  ...
                  return (
                    ...
                    >
                      <Icon className={`w-4 h-4 inline mr-2 ${priorityColors[p]}`} />
